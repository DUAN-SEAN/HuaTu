...
namespace HuaTu.Controls.Internal.Dockable.View
{
    ...
    /// </summary>
    internal partial class AutoHideButtonsPanel : ButtonContainer
    {
        #region Fields
        ...
        protected override void OnPaintPanelBackground(PaintEventArgs e)
        {
            ...
            if (dock == DockStyle.Top || dock == DockStyle.Bottom)
            {
                using (LinearGradientBrush backBrush =
                    new LinearGradientBrush(ClientRectangle, BackGradient1, BackGradient2, BackGradientMode))
                {
        ...
        private void ApplyDockBasedStyle(DockStyle dock)
        {
            switch (dock)
            {
                case DockStyle.Left:
                    Width = SizeLimit;
                    ButtonsRenderer = new RightUnitButtonDrawer();
                    break;

                case DockStyle.Top:
                    Height = SizeLimit;
                    ButtonsRenderer = new BottomUnitButtonDrawer();
                    break;

                case DockStyle.Right:
                    Width = SizeLimit;
                    ButtonsRenderer = new LeftUnitButtonDrawer();
                    break;

                case DockStyle.Bottom:
                    Height = SizeLimit;
                    ButtonsRenderer = new TopUnitButtonDrawer();
                    break;
            }
        }
